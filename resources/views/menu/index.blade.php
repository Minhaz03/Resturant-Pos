        ...
        <div class="card-body py-2">
            <form method="GET" class="row g-2 align-items-end">
                <div class="col-md-4"><input type="text" name="search" class="form-control form-control-sm"
                        placeholder="Search items..." value="{{ request('search') }}"></div>
                <div class="col-md-3">
                    ...
                    <tbody>
                        @forelse($menuItems as $item)
                            <tr>
                                ...
                                <td class="fw-semibold">৳{{ number_format($item->price, 2) }}
                                    @if ($item->discount > 0)
                                        <br><span class="badge bg-warning text-dark"
                                            style="font-size:0.7rem">{{ $item->discount }}% off</span>
                                    @endif
                                </td>
                                ...
                                <td><span class="badge {{ $item->is_available ? 'bg-success' : 'bg-secondary' }}"
                                        id="status-badge-{{ $item->id }}">{{ $item->is_available ? 'Active' : 'InActive' }}</span>
                                </td>
                                <td>
