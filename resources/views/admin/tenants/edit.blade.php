...
<div class="bg-gray-800 rounded shadow p-6 border border-gray-700 max-w-2xl">
    <form action="{{ route('admin.tenants.update', $tenant->id) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="email" name="email" value="{{ $tenant->email }}" class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white focus:outline-none focus:border-blue-500">
        </div>
        <div class="mb-4">
            <label class="block text-gray-300 text-sm font-bold mb-2">Restaurant Phone</label>
            <input type="text" name="phone" value="{{ $tenant->phone }}" class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white focus:outline-none focus:border-blue-500">
        </div>
        <div class="mb-4">
            <label class="block text-gray-300 text-sm font-bold mb-2">Restaurant Address</label>
            <textarea name="address" class="w-full bg-gray-700 border border-gray-600 rounded py-2 px-3 text-white focus:outline-none focus:border-blue-500" rows="3">{{ $tenant->address }}</textarea>
        </div>
        <div class="mb-4">
