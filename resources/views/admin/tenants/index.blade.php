...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full gap-4">
    <div>
        <h2 class="text-xl font-extrabold text-white flex items-center gap-2 tracking-tight">
            <i class="bi bi-shop text-blue-500"></i> Tenant Management
        </h2>
        <p class="text-xs text-slate-400 mt-0.5">Manage, monitor, and provision restaurant tenants across your multi-tenant ecosystem.</p>
    </div>
    <a href="{{ route('admin.tenants.create') }}" class="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-blue-600/30 transition-all flex items-center gap-2 shrink-0">
    ...
    <div class="bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-800 overflow-hidden">
        ...
        <div class="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/60">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-white">Registered Restaurants</h3>
                    <p class="text-xs text-slate-400">Total Provisioned Stores: {{ $tenants->total() }}</p>
                </div>
            </div>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="border-b border-slate-800 bg-slate-950/60 text-slate-400 text-[11px] uppercase tracking-wider">
                        <th class="py-3.5 px-5 font-semibold">Store ID</th>
                        <th class="py-3.5 px-5 font-semibold">Restaurant Store</th>
                        <th class="py-3.5 px-5 font-semibold">Subscription Plan</th>
                ...
                <tbody class="divide-y divide-slate-800/60 text-xs">
                    ...
                    <tr class="hover:bg-slate-800/40 transition-colors group">
                        <td class="py-3.5 px-5 font-mono text-slate-500">
                            #{{ str_pad($tenant->id, 4, '0', STR_PAD_LEFT) }}
                        </td>
                        <td class="py-3.5 px-5 font-semibold text-white">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center font-bold text-xs uppercase shrink-0">
                                    {{ strtoupper(substr($tenant->name, 0, 1)) }}
                                </div>
                                <div>
                                    <span class="text-slate-100 font-bold block">{{ $tenant->name }}</span>
                                    <span class="text-[10px] text-slate-500">{{ $tenant->email ?? 'No contact email' }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="py-3.5 px-5">
                            @if($tenant->subscriptions->isNotEmpty())
                                <span class="px-2.5 py-1 bg-blue-500/10 text-blue-300 rounded-lg border border-blue-500/20 font-semibold text-[11px]">
                                    <i class="bi bi-star-fill text-[9px] me-1 text-blue-400"></i>
                                    {{ $tenant->subscriptions->first()->plan->name ?? 'N/A' }}
                                </span>
                            @else
                                <span class="px-2.5 py-1 bg-slate-800 text-slate-500 rounded-lg border border-slate-700 text-[11px]">
                                    No Active Plan
                                </span>
                            @endif
                        </td>
                        ...
                        <td class="py-3.5 px-5 text-right">
                            <div class="inline-flex items-center gap-2">
                                <a href="{{ route('admin.tenants.show', $tenant->id) }}" class="w-8 h-8 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 flex items-center justify-center transition-colors" title="View Details">
                                    <i class="bi bi-eye"></i>
                                </a>
                                ...
                                <form action="{{ route('admin.tenants.destroy', $tenant->id) }}" method="POST" class="inline-block delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="w-8 h-8 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 border border-rose-500/20 flex items-center justify-center transition-colors btn-delete" title="Delete Tenant">
                    ...
                    <tr>
                        <td colspan="5" class="py-12 text-center text-slate-500">
                            ...
                            No tenant restaurants registered yet.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "You won't be able to revert this! Deleting a tenant removes all associated data and owner accounts.",
                    icon: 'warning',
