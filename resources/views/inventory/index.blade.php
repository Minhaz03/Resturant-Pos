...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        @can('create inventory')
            <a href="{{ route('inventory.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg me-1"></i>Add
                Inventory</a>
        @endcan
    </div>
    ...
    @if ($lowStockCount > 0)
        <div class="alert alert-warning d-flex align-items-center gap-2 mb-4">
            ...
            <strong>{{ $lowStockCount }} item(s)</strong>&nbsp;are below minimum stock level.&nbsp;<a
                href="?filter=low_stock" class="alert-link">View low stock items</a>
        </div>
    @endif
                    ...
                    <tbody>
                        @forelse($items as $item)
                            ...
                            <tr class="{{ $item->quantity == 0 ? 'table-danger' : ($isLow ? 'table-warning' : '') }}">
                                ...
                                <td>
                                    <span
                                        class="fw-bold {{ $item->quantity == 0 ? 'text-danger' : ($isLow ? 'text-warning' : 'text-success') }}">{{ number_format($item->quantity, 2) }}</span>
                                </td>
                                    ...
                                    <div class="d-flex gap-1">
                                        ...
                                        @can('edit inventory')
                                            <a href="{{ route('inventory.edit', $item) }}"
                                                class="btn btn-sm btn-outline-primary py-0 px-2"><i
                                                    class="bi bi-pencil"></i></a>
                                        @endcan
                                        @can('edit inventory')
                                            <button class="btn btn-sm btn-outline-success py-0 px-2" data-bs-toggle="modal"
                                                data-bs-target="#adjustModal{{ $item->id }}" title="Adjust Stock"><i
                                                    class="bi bi-arrow-repeat"></i></button>
                                        @endcan
                                        @can('delete inventory')
                                            <form method="POST" action="{{ route('inventory.destroy', $item) }}"
                                                data-confirm="Delete inventory item?" data-confirm-button="Yes, delete!">@csrf
                                    ...
                                    <div class="modal-dialog modal-sm">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 class="modal-title">Adjust: {{ $item->name }}</h6><button type="button"
                                            ...
                                            <form method="POST" action="{{ route('inventory.adjust', $item) }}">@csrf
                                                <div class="modal-body">
                                                    ...
                                                    <div class="mb-3"><label class="form-label">Quantity (+ add, -
                                                            remove)</label><input type="number" name="quantity"
                                                            class="form-control form-control-sm" step="0.01"
                                                            placeholder="e.g. 10 or -5" required></div>
                                                ...
                                                <div class="modal-footer"><button type="button"
                                                        ...
                                                        class="btn btn-sm btn-primary">Adjust</button></div>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="text-center py-4 text-muted">No inventory items found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
